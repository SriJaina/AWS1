...
namespace Azure_Automation
{
    ...
    public partial class TestClass
    {
        [Test]
        [TestMethod]
        public void DecomWindowsVMNoActiveDataDisk()
        {
            ExcelMethods em = new ExcelMethods();
            #region data Required
            String[] allSheet = ExcelMethods.getAllSheetName();
            DataSet ds = ExcelMethods.getDataSetForSheet("DecomWindowsVMNoActiveDataDisk");
            ...
            reuse.TryCatchMethod(appServ, appServEnv, FillDecomWindowsVMNoActiveDataDiskEnvironmentDetails, "User is able to fill details", "User is not able to fill all the details");
            reuse.TryCatchMethod(reuse.moveToNextPage, "Navigated to VM Details screen", "Unable to navigate to VM Details screen");
            reuse.TryCatchMethod(servers, FillDecomWindowsVMNoActiveDataDiskEnvironmentDetails, "User is able to fill details", "User is not able to fill all the details");
            reuse.TryCatchMethod(DecomWindowsVMNoActiveDataDiskValidation, "No data disk is displayed", "No data disk is not displayed since the selected VM contains active data disk");
        ...
        public void FillDecomWindowsVMNoActiveDataDiskEnvironmentDetails(string appServ, string appServEnv)
        {
        ...
        public void FillDecomWindowsVMNoActiveDataDiskEnvironmentDetails(string servers)
        {
            System.Threading.Thread.Sleep(3000);
        ...
        public void DecomWindowsVMNoActiveDataDiskValidation()
        {
